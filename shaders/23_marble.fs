/*
  23_marble.fs: uso implementazione simplex noise da
   https://github.com/ashima/webgl-noise/wiki
   Versione di 19b_turbulence_abs.fs che utilizza il valore di grigio della turbolenza
   come coordinata texture per leggere all'interno di una texture esterna contenente un gradiente 
   di colore.
   In questo modo un pattern di colori possono essere applicate sulla base di un pattern casuale.

NB) utilizzare "16_procedural_base.vs" come vertex shader
...
// variabile con i valori delle uv interpolate
varying vec2 vUv;
...
{  
  ...
  vec4 color = texture2D(tex, vec2(value, 0.0));
